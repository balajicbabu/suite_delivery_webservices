...
def main():
    try:
        test.log("Test Case Name: tst_create_parse_failure_delivery")

        test.log("create parse failure delivery")
        ...
        manifest_parse_success = None
        manifest_parse_failure = "vevo_status\\parse_failure\\status-manifest.xml"
        #manifest_parse_failure = findFile("testdata", "vevo_status\\parse_failure\\status-manifest.xml")
        test.log("manifest_parse_success : " + "None")
        test.log("manifest_parse_failure : " + manifest_parse_failure)
        ...
        x = None
        count = 3 # using count for FTP to transfer the status-manifest file as a trigger
        delivery_id = None
        initial_failure = False
        delivery_stage = None
        source(findFile("scripts", "create_delivery.py"))
        delivery_id = create_delivery_first_step(create_delivery_data_success, videoISRC, hub_status_config_url, create_delivery_url, get_delivery_status_url, manifest_parse_success, manifest_parse_failure, count, ftp_vevo_host, ftp_vevo_user, ftp_vevo_password)
        squish.snooze(60)
        ...
        if(delivery_stage == "FAILED" and initial_failure == False):
            test.passes("parse_failure is successful")
        else:
            test.fail("parse_failure delivery has failed, deliveryStage : ")
    except Exception as e:
        print e
        s = str(e)
        print sys.exc_info()
        test.log(s)
        test.fail("parse_failure has failed due to :" + s)
